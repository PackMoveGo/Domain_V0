...
const Footer=()=>{
  ...
  // Debug: Monitor status code changes
  useEffect(() => {
    console.log('Footer: Status code changed to:', statusCode);
  }, [statusCode]);
    ...
    const loadFooterData = async () => {
      try {
        ...
        try {
          ...
          console.log('Footer: Health status code from API:', healthStatusCode);
        } catch (healthError) {
          console.log('Footer: Health check failed, setting to 503:', healthError);
          healthStatusCode = 503;
        }
        
        // Check global 503 status as backup
        const isGlobal503 = isApiGlobally503();
        console.log('Footer: Global 503 status:', isGlobal503);
        
        // Set status code - prioritize 503 if either check indicates it
        if (healthStatusCode === 503 || isGlobal503) {
          console.log('Footer: Setting status to 503 (health:', healthStatusCode, 'global:', isGlobal503, ')');
          setStatusCode(503);
        } else {
          console.log('Footer: Setting status to:', healthStatusCode);
          setStatusCode(healthStatusCode);
        }
        
        // Only load footer data if status code is not 503
        if (healthStatusCode !== 503 && !isGlobal503) {
          console.log('Footer: Loading footer data...');
          const data = await getFooterData();
          setFooterData(data);
        } else {
          console.log('Footer: Skipping footer data load due to 503 status');
        }
      } catch (error) {
        console.error('Error loading footer data:', error);
        console.log('Footer: Setting status to 503 due to error');
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          ...
          <div className="text-center">
            ...
            <div className="text-gray-400 text-sm">
              {(() => {
                console.log('Footer Services render - statusCode:', statusCode, 'footerData:', footerData);
                console.log('Footer: Global 503 check:', isApiGlobally503());
                
                // Force 503 for testing - remove this after confirming it works
                if (isApiGlobally503()) {
                  console.log('Footer: FORCING 503 due to global 503 status');
                  return <p className="text-red-500 font-bold">503</p>;
                }
                
                // Always check for 503 first - this takes priority over everything else
                if (statusCode === 503) {
                  console.log('Footer: Displaying 503 status code');
                  return <p className="text-red-500 font-bold">503</p>;
                } else if (statusCode >= 500) {
                  console.log('Footer: Displaying 5xx status code:', statusCode);
                  return <p className="text-red-500 font-bold">{statusCode}</p>;
                } else if (footerData?.services && Array.isArray(footerData.services)) {
                  console.log('Footer: Displaying service count:', footerData.services.length);
                  return <p>{footerData.services.length}</p>;
                } else {
                  console.log('Footer: No data available, showing 0');
                  return <p>0</p>;
                }
              })()}
            </div>
